...
changelog:
2.0.3:
    - fixed obs parameters
    - fixed the script eating other parameters (hopefully?)
2.0.2:
...
-------------------
    ...
    -----------------
        ...
        The third is what the first argument is going to be compared against.
...
-------------------
    ...
    $getapi(url)
    $write(filepath,content) OR $write(filepath,content,succeed-msg,fail-msg)
    $mathif(equation) (this is done by means of evaluating as python code, so complex equations may fail.)
...
having problems? have questions? ideas/requests? head over to my discord at discord.gg/VKp6zrs
